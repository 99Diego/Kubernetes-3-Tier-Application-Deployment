...
def get_database(db_name):
    try:
        # Fix: Add db_name to connection string and include authSource
        CONNECTION_STRING = f"mongodb://{mongo_username}:{mongo_password}@{mongo_host}:{mongo_port}/{db_name}?authSource=admin"
        app.logger.info(f'Connecting to MongoDB at {mongo_host}:{mongo_port}')
